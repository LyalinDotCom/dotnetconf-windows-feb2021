...
namespace BabySmash
{
    public partial class App : Application
    {
        ...
        {
            DeviceInformationService _deviceInfo = new DeviceInformationService();

            // Show start screen
            StartScreenView startScreen = new StartScreenView();
            startScreen.ShowDialog();

            // Start to Intercept keys but only after splash screen so it stops messing with my ability to use XAML Hot Reload
            InterceptKeyStart();

            // launch game
            Controller.Instance.Launch(_deviceInfo.GetAvailableScreens());
        }
        ...
        private void InterceptKeyStart()
        {
            try
            {
